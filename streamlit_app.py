...
with st.sidebar:
    ...
    sidebar_radio = st.radio(
        ...
        options = ['About', 'เช็คว่าต้องจด VAT หรือยัง', 'คำนวณ VAT', 'Q&A'], 
        index = 0, 
        label_visibility = 'collapsed'
    )
...
if sidebar_radio == 'About':
    ...
    <div style="display: flex; justify-content: center;">
        <h2 style="display: flex; align-items: center;">
            <img src="data:image/png;base64,{base64.b64encode(open('icon.png', 'rb').read()).decode()}" width="30" style="margin-right:10px;"> About
        </h2>
    </div>
    """, 
    ...
    st.markdown(
    """\n
    **⚠️โปรแกรมนี้รองรับการคำนวณผ่าน Browser ในคอมพิวเตอร์เท่านั้น⚠️**
    """, unsafe_allow_html=True)
...
# )
    ...
    ผมทำโปรแกรมนี้ขึ้นมาเพราะผมก็เป็นผู้ขายของออนไลน์และเคยปวดหัวกับการคำนวณยื่น VAT ทุก ๆ เดือนมาก่อน

    หลายคนถูกปรับจากกรมสรรพากรเพราะไม่รู้ว่ายอดขายถึงเกณฑ์ต้องจด VAT และไม่รู้วิธีการคำนวณ VAT ให้ถูกต้อง 
    เหตุผลหนึ่งเป็นเพราะ ข้อมูลยอดขายจาก Shopee, Lazada, TikTok มีโครงสร้างที่แตกต่างกัน การคำนวณจึงยุ่งยากและใช้เวลานาน

    ---
    ##### 🔹 **เครื่องมือนี้ช่วยอะไรคุณได้บ้าง?**  
    ✅ **เช็คว่าต้องจด VAT หรือยัง** -> อัปโหลดไฟล์ยอดขาย ระบบจะช่วยสรุปยอดและรายงานผลให้\n
    ✅ **คำนวณ VAT อย่างง่าย** -> อัพโหลดไฟล์ยอดขายและใบเสร็จค่าธรรมเนียม จะได้รายงานค่า VAT ที่ต้องยื่น\n
    ✅ **ลดเวลาการทำงาน** -> ไม่ต้องเสียเวลานั่งคำนวณยอดยื่น VAT เองทุกเดือน

    ---
    💡 หวังว่าโปรแกรมนี้จะช่วยให้ผู้ขายที่ยอดถึงเกณฑ์จด VAT ลดเวลาปวดหัวกับการคำนวณได้นะครับ 
    ...
    if len([store_name for store_name in store_name_ls if store_name != '']) == store_number:
        ...
        if check_ls == []: #อัพโหลดข้อมูลครบแล้ว
            ...
            if st.session_state.vat_calculate_clicked:
                ...
                with st.form("my_form"):
                    st.markdown(f'<h5 style="text-align: center">&nbsp;&nbsp;📋 กรอกข้อมูลเพื่อรับไฟล์แนบการยื่น VAT ทาง Email 📋</h5>', unsafe_allow_html=True)
